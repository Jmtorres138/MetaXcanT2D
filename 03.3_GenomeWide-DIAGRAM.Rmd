...
rds.dir <- root.dir %&% "intermediate_files/diag/"
...
diag.path <- "intermediate_files/diag.z.impG_0.80.gtexV6p.txt.gz"
...
save_figures <- function(alpha,model){
  ...
  p.df <- cbind(dplyr::select(p.df,contains("Gene")),
...
df <- readRDS(rds.dir %&% "diag/sig.gene.TW.a05.bonfer.df.RDS")
genes <- p.EN.df$Gene
...
Summary of genome-wide results at FDR 5%
...
df.b <- filter(df.f, diag.mult.correct==TRUE)
length(unique(df$gene))
f <- select(df.f,one_of("gene","type"))[!duplicated(df.f$gene),]
table(f$type) # known 37, T2D 25, unknown 52
