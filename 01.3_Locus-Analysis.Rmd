...
build_ensid_ref <- function(){
  ...
  for (i in 1:length(Gene)){
    setTxtProgressBar(pb,i)
    ensid <- unique(na.omit(as.character(ens.df[i,2:dim(ens.df)[2]])))[1]
    Ensemble <- append(Ensemble,ensid)
    gr <- suppressMessages(get_tx_gr(ensid))
    if (is.na(gr)){
      chromo <- NA;tx.start  <- NA; tx.end  <- NA
    } else{
      chromo <- unique(as.character(seqnames(gr)))
      tx.start  <- min(start(ranges(gr)))
      tx.end  <- max(end(ranges(gr)))      
    }
...
write.table(x=locus.df,file=save.dir%&%"Ensemble-reference.txt",
            sep="\t",quote=FALSE,row.names=FALSE)
saveRDS(object=locus.df,file=rds.dir%&%"ens.ref.df.RDS")
