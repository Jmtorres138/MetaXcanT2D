...
pdf.dir <- "/Users/jtorres/Google Drive/Projects/MetaXcan/MetaXcanT2D/pdf.gtexV6p/diag_gera_meta-analysis/" 
...
table.ref.dir <- "/Volumes/im-lab/nas40t2/jason/projects/MetaXcan/results/tables/GTExV6p/"

table.dir <- "/Volumes/im-lab/nas40t2/jason/projects/MetaXcan/results/meta-analyses/DIAGRAM-GERA_ImpG_0.80_gtexV6p/tables/"




meta.name <- "DIAGRAM-GERA_ImpG_0.80_gtexV6p" 
meta.name.ref <- "DIAGRAM_ImpG_0.8_gtexV6p"
...
gcL.list <- gc_dfs(0.5)
z.EN.df <- gcL.list[[1]]; p.EN.df <- gcL.list[[2]]; m.EN.df <- makeMdf(0.5)
remove(gcL.list)
#z.EN.df <- makeZdf(0.5); p.EN.df <- makePdf(0.5)
#z.EN.df <- makeZdf(1) ; p.EN.df <- makePdf(1)
```
...
save(z.EN.df,p.EN.df,q.EN.df,m.EN.df,file=paste0(rds.dir,"metaXcan.results.Rdata"))
...
makeNSNPSMODdf <- function(alpha){
  ...
  dir <- table.ref.dir %&% meta.name.ref %&% "."
...
Here, I will explore the top genes from the MetaXcan analyses and consider the affect of applying different restrictions, namely elasticNet models for **CrossTissue** and **Tissue-wide** predictors
...
After analyzing the plot created in the previous chunk of code, I will save dataframes for the TW.CT.a05  to carry foward for Gene Set Enrichment Analyses 

```{r save_dfs, echo=FALSE}
sig.gene.TW.a05.df <- sub_sig(TwCtOnly=TRUE,alphalimit=1) 
...
After analyzing the plot created in the previous chunk of code, I will save dataframes for the TW.CT.a05 and t2d.TW.a05 subsets to carry foward for Gene Set Enrichment Analyses 

```{r save_dfs_bonfer, echo=FALSE}
sig.gene.TW.a05.bonfer.df <- sub_sig(TwCtOnly=TRUE,bonfer=TRUE,alphalimit=1) 
