...
root.dir <- "./"
rds.dir <- root.dir %&% "intermediate_files/"
...
get_bonfer_sig_meta <- function(){
  load(rds.dir %&% "meta/metaXcan.results.Rdata")
  p.df <- p.EN.df
  p.df <- cbind(select(p.df,one_of("Gene","CrossTissue")),
                select(p.df,contains("TW_")))
  vec <- c()
...
loci.df <- (readRDS("intermediate_files/loci.df.RDS")) %>% select(chrom:nearby.txend)
...
get_nhgri_ebi_t2d_genes <- function(){
  file.path <- "input_files/nhgri_ebi-trait-genes.txt.gz"
...
build_table_core_df <- function(){
  ...
  gera.rep.df <- readRDS(rds.dir %&% "diag/sig.genes.gera-rep.df.RDS")
  diag.bonfer <- c()
  diag.mult.correct <- c() 
  meta.z.score <- c()
  meta.p.value <- c()
...
manual_entry <- function(mygene,mod,T2Dcore.df){
  sub.df <- filter(T2Dcore.df,gene==mygene)[1,]
  # DIAGRAM lookup 
  load(rds.dir%&%"diag/metaXcan.results.Rdata")
...
append_GWfdr05 <- function(T2Dcore.df){
  ...
  df1 <- manual_entry("CCNE2","TW_Cells_EBV-transformed_lymphocytes",T2Dcore.df)
  df2 <- manual_entry("HCG27","TW_Lung",T2Dcore.df)
  df3 <- manual_entry("TCP11L1","TW_Muscle_Skeletal",T2Dcore.df)
  df4 <- manual_entry("AP3S2","TW_Prostate",T2Dcore.df)
  df5 <- manual_entry("FAM229B","TW_Skin_Not_Sun_Exposed_Suprapubic",T2Dcore.df)
  df5$chrom <- 6
  df5$gene <- "FAM229B"
  df5$type <- "Unknown"
