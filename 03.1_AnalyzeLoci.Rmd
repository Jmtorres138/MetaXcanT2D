...
**Need to harmonize loci.df with data in p.EN.df. Importantly, values corresponding to genes with cross-validation r2 < 1% in a given tissue model had values set to NA. Need to esure that data in loci.df is consistent.** 

```{r loci_df,  echo=FALSE}


#harmonize_loci_df <- function(){
#  loci.df <- readRDS(rds.dir%&%"diag/t2d.loci.z.df.RDS")
...
loci.df <- readRDS(rds.dir%&%"diag/t2d.loci.z.df.RDS")
loci.df <- saveRDS(loci.df,"intermediate_files/loci.df.RDS")
loci.df <- readRDS(rds.dir%&%"diag/t2d.loci.z.df.RDS")
...
build_bonfer_df <- function(analysis.name){
  if (analysis.name=="DIAGRAM"){
    load("intermediate_files/diag/metaXcan.results.Rdata")
  } else if (analysis.name=="GERA-T2D"){
    load("intermediate_files/gera/metaXcan.results.Rdata")
  } else if (analysis.name=="Meta-analysis"){
    load("intermediate_files/meta/metaXcan.results.Rdata")
  } else{
...
loci.sig.df <- suppressWarnings(build_sig_locidf(gene.vec))
saveRDS(loci.sig.df,"intermediate_files/loci.sig.df.RDS")
loci.sig.df <- readRDS("intermediate_files/loci.sig.df.RDS")
...
print_report_nomultsig <- function(){
  ...
  saveRDS(save.list,"intermediate_files/loci.profile.list.RDS")
}
...
print_report_multsig <- function(){
  ...
  save.list <- list(rep.gene.sig.only.vec,rep.gene.sig.not.only.vec,
                      rep.gene.not.sig.only.vec,rep.gene.not.sig.not.only.vec)
  print("Saving list as RDS")
  saveRDS(save.list,"intermediate_files/loci.profile.multcorrect.list.RDS")
}
...
print_report_loci <- function(){
  ...
  save.list <- list(rep.gene.sig.only.vec,rep.gene.sig.not.only.vec,
                      rep.gene.not.sig.only.vec,rep.gene.not.sig.not.only.vec)
  print("Saving list as RDS")
  saveRDS(save.list,"intermediate_files/loci.profile.multcorrectLoci.list.RDS")
}

suppressWarnings(print_report_nomultsig())
suppressWarnings(print_report_multsig())
suppressWarnings(print_report_loci())
```
