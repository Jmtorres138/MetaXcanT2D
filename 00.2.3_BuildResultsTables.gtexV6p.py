...
meta_name = sys.argv[1] # DIAGRAM_ImpG_0.8_gtexV6p # GERA_ImpG_0.8_gtexV6p
...
def build_gene_dic():
    ...
    for tiss in tiss_list:
        ...
        for line in fin:
            l = line.strip().split(',')
            ensid,gene,zscore,pval,cvr2,n,ntot,cvp = l[0],l[1],l[2],l[4],l[6],l[9],l[11],l[7]
            gene_list.append(gene)
            try:
                gene_dic[gene].append([tiss,zscore,pval,cvr2,n,ntot,ensid,cvp])
            except:
                gene_dic[gene] = [[tiss,zscore,pval,cvr2,n,ntot,ensid,cvp]]
        fin.close()
...
def write_outfile6(gene_list,gene_dic):
    print("Writing Ensemble Gene ID table...")
    fout = gzip.open(out_dir+meta_name+"."+str(alpha)+".ensid.table.csv.gz",'wb')
    head_list = ["Gene"] + tiss_list
    fout.write(",".join(head_list)+"\n")
    for gene in gene_list:
        write_list = []
        ...
        for tiss in tiss_list:
            ...
            if check != True:
                ensid = "NA"
            write_list.append(ensid)
        fout.write(",".join(write_list)+"\n")
    fout.close()

def write_outfile7(gene_list,gene_dic):
    print("Writing prediction performance p-value result table...")
    fout = gzip.open(out_dir+meta_name+"."+str(alpha)+".ppp.table.csv.gz",'wb')
    head_list = ["Gene"] + tiss_list
    fout.write(",".join(head_list)+"\n")
    for gene in gene_list:
        write_list = []
...
def main():
    ...
    write_outfile6(gl,gd)
    write_outfile7(gl,gd)
